...
type Ctx = { items: CartItem[]; add: (e: Event, qty?: number) => void; remove: (id: string) => void; clear: () => void; total: number }
...
export function CartProvider({children}:{children:React.ReactNode}){
  ...
  const add=(event:Event,qty=1)=>setItems(p=>{const i=p.findIndex(x=>x.event.id===event.id); if(i>=0){const c=[...p]; c[i]={event, qty:c[i].qty+qty}; return c} return [...p,{event,qty}]})
...
export function useCart(){ const ctx=useContext(Cart); if(!ctx) throw new Error('useCart must be used within CartProvider'); return ctx }